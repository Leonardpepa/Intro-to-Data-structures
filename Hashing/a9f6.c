...
typedef int ListElementTypeB;

typedef struct {
	int RecKey;
	ListElementTypeB Data;
	ListElementType Data2;
	int Link;
} ListElm;
...
void SearchVisit(HashListType DB){
    ...
    scanf("%s",&Client);
    getchar();
Key=GetCharacterSum(Client);

    SearchHashList(DB, Key, &Loc, &Pred);
    if(Loc == -1)
        printf("There's no client named %s.\n",Client);
    else
        {
        printf("Service: %d\n",DB.List[Loc].Data2.service);
        printf("Amount Paid: %.2lf\n",DB.List[Loc].Data2.payment);
        }
printf("\n");
}
